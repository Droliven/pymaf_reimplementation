...
from ..nets import SMPL

class PartRenderer():
    """Renderer used to render segmentation masks and part segmentations.
    Internally it uses the Neural 3D Mesh Renderer
    """
    def __init__(self, focal_length=5000., render_res=224, JOINT_MAP=None, JOINT_NAMES=None, J24_TO_J19=None, JOINT_REGRESSOR_TRAIN_EXTRA=None, SMPL_MODEL_DIR=None, VERTEX_TEXTURE_FILE=None, CUBE_PARTS_FILE=None):
        ...
        self.neural_renderer = nr.Renderer(dist_coeffs=None, orig_size=self.render_res,
                                           ...
                                           anti_aliasing=False)
        self.faces = torch.from_numpy(SMPL(JOINT_MAP, JOINT_NAMES, J24_TO_J19, JOINT_REGRESSOR_TRAIN_EXTRA, SMPL_MODEL_DIR).faces.astype(np.int32)).cuda()
        textures = np.load(VERTEX_TEXTURE_FILE)
        self.textures = torch.from_numpy(textures).cuda().float()
        self.cube_parts = torch.cuda.FloatTensor(np.load(CUBE_PARTS_FILE))
