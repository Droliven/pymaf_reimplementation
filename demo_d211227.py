...
'''
@project : pymaf_reimp
@file    : demo_d211227.py
@author  : Levon
@contact : levondang@163.com
@ide     : PyCharm
@time    : 2022-01-04 10:35
'''
...
from d211227_pymaf_reimp.nets import PyMAF, SMPL
from d211227_pymaf_reimp.datas import Inference
from d211227_pymaf_reimp.utils.renderer import OpenDRenderer, PyRenderer
from d211227_pymaf_reimp.utils.imutils import crop
from d211227_pymaf_reimp.utils.demo_utils import (
    download_url,
...
def run_image_demo():
    ...
    model = PyMAF(cfg.pymaf_model['BACKBONE'], cfg.res_model['DECONV_WITH_BIAS'],
                       ...
                       data_dir=cfg.preprocessed_data_dir).to(cfg.device)
    ...
    if args.use_opendr:
        renderer = OpenDRenderer(resolution=(cfg.IMG_RES, cfg.IMG_RES), JOINT_MAP=cfg.JOINT_MAP, JOINT_NAMES=cfg.JOINT_NAMES, J24_TO_J19=cfg.J24_TO_J19, JOINT_REGRESSOR_TRAIN_EXTRA=cfg.JOINT_REGRESSOR_TRAIN_EXTRA, SMPL_MODEL_DIR=cfg.SMPL_MODEL_DIR)
    else:
        renderer = PyRenderer(resolution=(cfg.IMG_RES, cfg.IMG_RES), JOINT_MAP=cfg.JOINT_MAP, JOINT_NAMES=cfg.JOINT_NAMES, J24_TO_J19=cfg.J24_TO_J19, JOINT_REGRESSOR_TRAIN_EXTRA=cfg.JOINT_REGRESSOR_TRAIN_EXTRA, SMPL_MODEL_DIR=cfg.SMPL_MODEL_DIR)

    # Preprocess input image and generate predictions
    img_np, img, norm_img = process_image(args.img_file, input_res=cfg.IMG_RES)
    with torch.no_grad():
        preds_dict, _ = model(norm_img.to(cfg.device))
    ...
    camera_translation = torch.stack([pred_camera[:, 1], pred_camera[:, 2],
                                      2 * cfg.FOCAL_LENGTH / (cfg.IMG_RES * pred_camera[:, 0] + 1e-9)],
                                     dim=-1)
...
def run_video_demo():
    ...
    if args.recon_result_file:
        ...
        for person_id in tqdm(list(tracking_results.keys())):
            ...
            dataloader = DataLoader(dataset, batch_size=args.model_batch_size, num_workers=cfg.num_works)
    ...
    if not args.no_render:
        ...
        if args.use_opendr:
            renderer = OpenDRenderer(resolution=(orig_height, orig_width), JOINT_MAP=cfg.JOINT_MAP, JOINT_NAMES=cfg.JOINT_NAMES, J24_TO_J19=cfg.J24_TO_J19, JOINT_REGRESSOR_TRAIN_EXTRA=cfg.JOINT_REGRESSOR_TRAIN_EXTRA, SMPL_MODEL_DIR=cfg.SMPL_MODEL_DIR)
        else:
            renderer = PyRenderer(resolution=(orig_width, orig_height), JOINT_MAP=cfg.JOINT_MAP, JOINT_NAMES=cfg.JOINT_NAMES, J24_TO_J19=cfg.J24_TO_J19, JOINT_REGRESSOR_TRAIN_EXTRA=cfg.JOINT_REGRESSOR_TRAIN_EXTRA, SMPL_MODEL_DIR=cfg.SMPL_MODEL_DIR)
        ...
        if cfg.pymaf_model.N_ITER == 0 and cfg.pymaf_model.AUX_SUPV_ON == False:
            color_type = 'neutral'
        else:
            color_type = 'purple'
        ...
        for frame_idx in tqdm(range(len(image_file_names))):
            ...
            if args.render_ratio != 1:
                img = resize(img, (int(img.shape[0] * args.render_ratio), int(img.shape[1] * args.render_ratio)),
                             anti_aliasing=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_opendr', action='store_true', help='Use opendr to render the predicted SMPL instead of pyrender')
    parser.add_argument('--with_raw', action='store_true', help='attach raw image.')
    parser.add_argument('--empty_bg', action='store_true', help='render meshes on empty background.')
    # parser.add_argument('--sideview', action='store_true', help='render meshes from alternate viewpoint.')
    parser.add_argument('--image_based', action='store_true', help='image based reconstruction.')
    parser.add_argument('--use_gt', action='store_true', help='use the ground truth tracking annotations.')
    ...
    parser.add_argument('--recon_result_file', type=str, default='', help='path to reconstruction result file.')
    parser.add_argument('--pre_load_imgs', action='store_true', help='pred-load input images.')
    parser.add_argument('--save_obj', action='store_true', help='save results as .obj files.')

    parser.add_argument('--run_img', type=bool, default=True, help='Path to a single input image')

    parser.add_argument('--img_file', type=str, default=r"./assets/examples/COCO_val2014_000000019667.jpg", help='Path to a single input image')
    parser.add_argument('--vid_file', type=str, default=r"./assets/examples/flashmob.mp4", help='input video path or youtube link')
    ...
    parser.add_argument('--output_folder', type=str, default=osp.abspath("./results"), help='output folder to write results')
    parser.add_argument('--image_folder', type=str, default=None, help='input image folder')


    args = parser.parse_args()
    cfg = ConfigPymaf(exp_name="demo", is_debug=False)
    print("\n================== Arguments =================")
